        ...
        <div class="kk-stat-card">
            ...
            <div class="row g-2">
                <div class="col-6 col-md-3">
                    <a href="{{ route('kasir.orders.create') }}" class="kk-qa-card" style="--qa-accent:#f59e0b; --qa-glow:rgba(245,158,11,0.35);">
                ...
                <div class="col-6 col-md-3">
                    <a href="{{ route('kasir.stock_in.index') }}" class="kk-qa-card" style="--qa-accent:#38bdf8; --qa-glow:rgba(56,189,248,0.35);">
                ...
                <div class="col-6 col-md-3">
                    <a href="{{ route('kasir.stock_out.index') }}" class="kk-qa-card" style="--qa-accent:#f43f5e; --qa-glow:rgba(244,63,94,0.35);">
                ...
                <div class="col-6 col-md-3">
                    <a href="{{ route('kasir.stock_harian.index') }}" class="kk-qa-card" style="--qa-accent:#10b981; --qa-glow:rgba(16,185,129,0.35);">
        ...
        <div class="kk-stat-card">
            ...
            @if($stokRendah > 0)
                <div class="d-flex flex-column flex-md-row gap-3 flex-wrap">
                    @foreach(\App\Models\Item::where('master_location', \App\Models\Item::MASTER_KASIR)->whereHas('stockIns')->get()->filter(fn($i) => $i->stokKasir() <= 10)->take(5) as $item)
                        <div class="d-flex align-items-center justify-content-between p-3 rounded flex-fill" style="background: var(--kk-danger-soft); border: 1px solid rgba(239, 68, 68, 0.2);">
                            <div>
                                ...
                                <div style="font-size:0.85rem; color:var(--kk-danger);">Sisa: {{ $item->stokKasir() }} {{ $item->unit }}</div>
                            </div>
                            <a href="{{ route('kasir.orders.create', ['item_id' => $item->id]) }}" class="btn btn-sm btn-outline-danger" style="font-weight: 500;">Restock</a>
                        </div>
                    @endforeach
                </div>
            @else
