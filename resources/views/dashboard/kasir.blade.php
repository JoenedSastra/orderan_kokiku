        ...
        <div class="kk-stat-card flex-fill">
            ...
            @if($stokRendah > 0)
                <div class="d-flex flex-column gap-2">
                    @foreach(\App\Models\Item::where('min_stock', '>', 0)->where('master_location', \App\Models\Item::MASTER_KASIR)->get()->filter(fn($i) => $i->stokKasir() <= $i->min_stock)->take(5) as $item)
                        <div class="d-flex align-items-center justify-content-between p-2 rounded" style="background: var(--kk-danger-soft); border: 1px solid rgba(239, 68, 68, 0.2);">
                            <div>
                                ...
                                <div style="font-size:0.75rem; color:var(--kk-danger);">Sisa: {{ $item->stokKasir() }} {{ $item->unit }}</div>
                            </div>
                            <a href="{{ route('kasir.orders.create', ['item_id' => $item->id]) }}" class="btn btn-sm btn-outline-danger" style="font-size:0.7rem; padding:0.2rem 0.5rem;">Restock</a>
                        </div>
                    @endforeach
                </div>
            @else
