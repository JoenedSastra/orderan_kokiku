...
<div class="dropdown kk-notif-dropdown">
    <button class="btn position-relative" type="button"
            ...
            title="Notifikasi">
        @if($unreadCount > 0)
            <span class="kk-bell-ring"><i class="bi bi-bell-fill" style="color:#ef4444;"></i></span>
                    ...
                    <div class="d-flex justify-content-end gap-1 flex-wrap">
                        @if($unreadCount > 0)
                            <form method="POST" action="{{ route('notifications.read-all') }}" class="m-0 d-inline-block">
                                @csrf
                                <button type="submit" class="btn text-white px-2 py-1" style="background-color: {{ auth()->user()->role === 'kasir' ? '#ef4444' : '#1e40af' }}; font-size:0.75rem; border-radius: 4px; border: none; white-space: nowrap; width: max-content !important; display: inline-block;">
                                    Tandai dibaca
                                </button>
                            </form>
                        @endif
        ...
        <div class="dropdown-item position-relative d-flex align-items-center p-0 {{ $notif->read_at ? '' : 'kk-notif-unread' }}" style="padding-right: 2.5rem !important;">
            <button type="button" class="kk-notif-modal-btn w-100 text-start bg-transparent border-0 p-3"
                ...
                <div class="d-flex gap-2 align-items-start">
                    <div style="width:30px; height:30px; border-radius:50%; background:rgba(239, 68, 68, 0.12); color:#ef4444; display:flex; align-items:center; justify-content:center; font-size:0.8rem; flex-shrink:0; margin-top:1px;">
                        <i class="bi bi-bell"></i>
                    </div>
                    ...
                    @if(!$notif->read_at)
                        <div style="width:7px; height:7px; background:#ef4444; border-radius:50%; flex-shrink:0; margin-top:5px;"></div>
                    @endif
                </div>
            </button>
